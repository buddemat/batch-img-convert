...
def get_args():
    ...
    parser.add_argument('-r', '--recursive', help='traverse subfolders of inpath',
                               action='store_true')
    parser.add_argument('-p', '--pool', type=int, help='poolsize for data parallelism. if option string is present without argument, then half the number of cpus are used.', const=cpucount//2, default=1, nargs='?')
